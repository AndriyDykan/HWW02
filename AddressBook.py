...
class Events:
    ...
    @abstractmethod
    def __get_current_week(self, *args):
        pass
...
class APIbook(Events):
    ...
    def load(self, file_name):
        ...
        if emptyness.st_size != 0:
            with open(file_name + '.bin', 'rb') as file:
                self.module.data = pickle.load(file)
    ...
    def search(self, pattern, category):
        ...
        for account in self.module.data:
    ...
    def remove(self, pattern):
        ...
        for account in self.module.data:
            if account['name'] == pattern:
                self.module.data.remove(account)
